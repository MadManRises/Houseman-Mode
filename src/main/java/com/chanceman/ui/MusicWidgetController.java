...
public class MusicWidgetController
{
    ...
    private void applyOverride(NpcDropData dropData)
    {
        ...
        Set<Integer> unlockedIds = unlockedItemsManager.getUnlockedItems();
        int totalDrops = drops.size();
        int unlockedCount = (int) drops.stream()
                .filter(d -> unlockedIds.contains(d.getItemId()))
                .count();
        ...
        if (root != null)
        {
            ...
            int fillW = Math.round(innerWidth * (float) unlockedCount / totalDrops);
            ...
            String txt = String.format("%d/%d", unlockedCount, totalDrops);
        ...
        for (DropItem d : drops)
        {
            ...
            icon.setOpacity(unlockedIds.contains(itemId) ? 0 : 150);
            icon.revalidate();
