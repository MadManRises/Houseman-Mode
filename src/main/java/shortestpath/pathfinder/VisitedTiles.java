...
public class VisitedTiles {
    ...
    public boolean get(int x, int y, int plane) {
        final VisitedRegion region = getRegion(x, y);
        if (region == null) {
            return false;
        }
    ...
    public VisitedRegion getRegion(int x, int y){
        final int regionIndex = getRegionIndex(x / REGION_SIZE, y / REGION_SIZE);
        if (regionIndex < 0 || regionIndex >= visitedRegions.length) {
            return null; // Region is out of bounds; report that it's been visited to avoid exploring it further
        }

        return visitedRegions[regionIndex];
    }
